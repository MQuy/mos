...
then
  ...
  if [ "$2" == "iso" ]
  then
    sudo qemu-system-i386 -s -S -boot c -cdrom mos.iso -hda hdd.img -netdev tap,id=mynet0,ifname=tap0,script=./tapup.sh,downscript=./tapdown.sh -device rtl8139,netdev=mynet0,mac=52:55:00:d1:55:01
  else
