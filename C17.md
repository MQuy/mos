...
- zero with bit field is only allowed for nameless and specify the next bit field will begin at a next unit type
  ...
  ```
- `register/restrict` and `inline` (only for function) are the hint, compiler is free to ignore it
...
- for `static inline`, if there is no reference, compiler can ignore that function in resuling object file (each translation unit will have each own defintion of that function -> waste space). While `extern inline`, compiler has to generate global definition in resulting object file (only one global definition). [`inline` alone can lead to confusing thing](https://blogs.oracle.com/d/inline-functions-in-c)
- an empty list in a function declaration means a function has unspecified number of parameters, while an empty list in a function definition means a function has no parameters
  ```c
  int foo();
  int main() {
...
- `typedef` can be interpreted different based on contexts
  ...
  ```
- `typedef` denotes a variable length array type, the length of array is fixed at the time `typedef` is defined
  ```c
  void copyt(int n)
  {
...
- in the initializer, if number of initialized elements exceed size array, compiler freely discard those
  ...
  ```
- if the initializer of subobject doesn't begin with a left brace, subobject will takes enough initializers from list and left remaining initializers for next element
  ```c
  struct foo
  {
  	int x, y, z;
...
- source file inclusion has 3 forms
  ...
  ```
- `#` follows by an identifer, the identifer is not subject to macro replacement
